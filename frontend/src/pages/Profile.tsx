...
import { getUserById, getContents, getMemesByUser, getFollowCounts, isFollowing, toggleFollow, getOrCreateInviteCode, getInviteStats } from '../lib/api/client'
...
export default function Profile({ user, setUser: _setUser }: { user: any; setUser: (u: any) => void }) {
  ...
  const fetchMyContents = async (uid: string) => {
    ...
    const [memes, contents] = await Promise.all([
      getMemesByUser(uid, 100),
      getContents(100, 0, undefined, uid),
    ])
    const memesWithSource = (memes || []).map(m => ({ ...m, _source: 'memes' }))
    const contentsWithSource = (contents || []).map(c => ({ ...c, _source: 'contents' }))
    setMyContents([...memesWithSource, ...contentsWithSource].sort((a, b) =>
