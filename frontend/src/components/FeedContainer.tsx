...
export default function FeedContainer({ contents, user, onMeme, onLoadMore, loadingMore }: FeedContainerProps) {
  ...
  const goTo = (i: number) => { if (!modalOpen && i >= 0 && i < contents.length) setCurrentIndex(i) }
  const goNext = () => {
    if (modalOpen) return
    if (currentIndex < contents.length - 1) {
      setCurrentIndex(currentIndex + 1)
      if (currentIndex >= contents.length - 10 && onLoadMore) onLoadMore()
    }
  }
  const goPrev = () => { if (!modalOpen) goTo(currentIndex - 1) }

  const handleTouchStart = (e: React.TouchEvent) => { if (!modalOpen) touchStartY.current = e.touches[0].clientY }
  const handleTouchMove = (e: React.TouchEvent) => { if (!modalOpen) touchEndY.current = e.touches[0].clientY }
  const handleTouchEnd = () => {
    if (modalOpen) return
    const d = touchStartY.current - touchEndY.current
