#!/usr/bin/env python3
"""Create blind box tables - run from server dir where node_modules lives"""
import paramiko, io
...
# Upload runner to server dir (near node_modules)
node_script = """
const fs = require('fs');
...
"""
sftp.putfo(io.BytesIO(node_script.encode('utf-8')), '/opt/promotion-mvp/server/_run_blind_sql.cjs')
sftp.close()

# Execute from server dir
stdin, stdout, stderr = client.exec_command(
    'cd /opt/promotion-mvp/server && /usr/local/lighthouse/softwares/nodejs/node/bin/node _run_blind_sql.cjs 2>&1',
    timeout=30
)
